set -e
docker build -t rdhyee/evernote-oauth .
...
# copy the certs
docker cp server.crt certs:/etc/nginx/certs/evernote-oauth.ydns.eu.crt
docker cp server.key certs:/etc/nginx/certs/evernote-oauth.ydns.eu.key

# setup the environment
source setenv.sh

# jupyter stack images (don't map local drive)
docker run -d --name evernote  \
   -e VIRTUAL_HOST=evernote-oauth.ydns.eu \
   -e VIRTUAL_PORT=5000 \
